app.controller('classRoomAlloCtrl', ['$scope','$rootScope','$localStorage','$location','$timeout','$http','$modal','editableOptions', 'editableThemes','toaster','$ngBootbox',
  ...
  {name: 'Class 1', code: 'ct001'},
  {name: 'Class 2', code: 'ct002'},
  {name: 'Class 3', code: 'ct003'},
  ];
  ...
  {name: 'Batch 1', code: 'ct001'},
  {name: 'Batch 2', code: 'ct002'},
  {name: 'Batch 3', code: 'ct003'},
  ];
  $scope.selectSlitfunction1 = function(selectedData) {
    if (angular.isObject(selectedData)) {
      return selectedData.name;  
    }else{
      return selectedData;  
  ...
  $scope.addNewCategory = function() {
    ...
    $http.get($rootScope.endUrl+'StudentAPI/studentAdmissionViews',{headers:{'access_token':$scope.access_token}}).success(function(incomingData) {
      // alert(incomingData);
